...
def move_ants_history(fund_code, page_index=1):
    ...
    try:
        ...
        if not data or data.get("Data") is None or "LSJZList" not in data["Data"]: return local_data, 0, "限流"
    ...
    except Exception as e: return local_data, 0, f"断流({str(e)})"
...
elif op_mode == "🔄 🛠️ 智能一键搬家":
    st.markdown("#### 🚀 全员多页联动抗反爬增量搬运")
    max_pages = st.slider("🎚️ 请选择本次全员搬运的深度（页数）：", min_value=1, max_value=5, value=2)
    if st.button(f"🔥 启动：全员自动追溯"):
...
if current_db:
    ...
    if not df_filtered.empty:
        ...
        with c_axis1: is_manual_y = st.checkbox("🔒 启用手动锁定纵轴", value=False)
        ...
        with c_axis2: manual_min = st.number_input("📉 自定义 Y 轴下限", value=round(current_min - padding, 2), step=0.02, disabled=not is_manual_y)
        with c_axis3: manual_max = st.number_input("📈 自定义 Y 轴上限", value=round(current_max + padding, 2), step=0.02, disabled=not is_manual_y)

        df_melted = df_filtered.melt(
            id_vars=['日期'], value_vars=['单位净值', '月度平均价中枢', '当月最高价边界'],
            var_name='指标类型', value_name='净值数值'
        )

        y_scale = alt.Scale(domain=[manual_min, manual_max], clamp=True) if is_manual_y else alt.Scale(zero=False, padding=15)
        color_scale = alt.Scale(domain=['单位净值', '月度平均价中枢', '当月最高价边界'], range=['#1F77B4', '#25A15C', '#FF4B4B'])
