...
# 🌟 核心改进：初始配置不包含死策略文本，全部由最新的估值百分位动态演算生成
DEFAULT_CONFIG = {
    '008163': {
...
def evaluate_investment_strategy(pe_percent, period, amount):
    ...
    if pe_percent >= 75.0:
        level = "high"
        status = "估值显著偏高 · 风险聚集区"
        if "每" in period or "天" in period:
            strategy = f"🚨 当前估值百分位（{pe_percent:.1f}%）已突破高危防线。建议收缩防线，将定投扣款暂调至【每日 {half_amount}元】左右，或分批落实止盈策略，保留现金火种。"
        else:
            strategy = f"⚠️ 当前估值百分位（{pe_percent:.1f}%）处于历史高位区间。策略上建议【维持当前基本定投，严禁任何主动加仓行为】，高溢价指数可考虑逐步锁定利润。"
            
    elif pe_percent <= 35.0:
        level = "low"
        status = "深度低估 · 黄金击球区"
        strategy = f"💎 估值百分位处于深度低估（{pe_percent:.1f}%）核心区！安全边际极高，属于绝对战略加仓点，请坚定不移执行当前 {plan_text}，资金宽裕者可考虑越跌越买，提升至【{period} {double_amount}元】。"
        
    else:
        level = "mid"
        status = "中性均衡 · 动态蓄力区"
        strategy = f"⚖️ 当前估值百分位（{pe_percent:.1f}%）处于中性均衡水位。上行有压力，下行有支撑，建议【严格不折不扣地执行常规既定计划 {plan_text}】，不盲目加仓，亦不轻易断供。"
...
def move_ants_front_latest(fund_code):
    ...
    except: return local_data, 0, "网络异动"
    
    if not raw_list: return local_data, 0, "无数据"
    new_count = 0
    ...
    return local_data, new_count, "前端齐平"
...
def move_ants_deep_history_v2(fund_code, max_pages=2):
    ...
    if total_new_inserted > 0:
        local_data = sorted(local_data, key=lambda x: x['日期'], reverse=True)
        save_local_history(fund_code, local_data)
    return local_data, total_new_inserted, f"穿透扫描 {pages_searched} 页 (扩充新空白 {pages_actually_saved} 页)"
...
<div class="dash-header">
    <h1>📊 定投监控看板</h1>
    <div class="subtitle">
        {len(st.session_state.fund_config)} 只核心监控资产 &nbsp;·&nbsp; 本月预计定投流水 <span style="color:#F0A500;font-weight:600">{total_monthly:,.0f}</span> 元<br>
        📈 智能估值战略裁判引擎已接管全线策略
    </div>
</div>
""", unsafe_allow_html=True)
...
for code, info in st.session_state.fund_config.items():
    # 🌟 核心运行：根据当前的百分位、周期和金额，现场计算出状态和策略描述
    lvl, status_text, strategy_text = evaluate_investment_strategy(
        pe_percent=info['pe_percent'], 
        period=info['period'], 
        amount=info['amount']
    )
...
op_mode = st.radio("系统功能切换", ["📝 修改与更新估值", "🔄 智能搬家", "📋 批量导入", "➕ 增删管理"], horizontal=True)

st.markdown('<div class="console-card">', unsafe_allow_html=True)

if op_mode == "📝 修改与更新估值":
    with st.form("edit_form"):
        st.markdown(f"**📝 正在更新 [{edit_code}] 的核心参数面板**")
        c1, c2 = st.columns(2)
        ...
        st.caption("🔍 填入市场最新的估值数据，上方的定投智能建议会基于策略算法全自动同步重算更新。")
        c3, c4 = st.columns(2)
        with c3: new_pe_ttm = st.number_input("最新 PE (TTM)", value=float(current_info['pe_ttm']), step=0.01, format="%.2f")
        with c4: new_pe_pct = st.number_input("最新 PE 百分位（%）", value=float(current_info['pe_percent']), step=0.01, format="%.2f")
        new_div = st.text_input("最新 TTM 股息率", value=current_info['div_yield'])
        
        if st.form_submit_button("💾 立即覆盖保存配置并触发智能联动"):
            st.session_state.fund_config[edit_code].update({
                'period': new_period, 'amount': new_amount, 'pe_ttm': new_pe_ttm, 'pe_percent': new_pe_pct, 'div_yield': new_div
            })
            save_config(st.session_state.fund_config)
            st.success("✅ 配置保存成功！决策引擎已根据最新估值点重新推演策略建议。")
            st.rerun()

elif op_mode == "🔄 智能搬家":
    st.markdown("**双向智能历史穿透吞噬系统**")
    if 'migration_log' in st.session_state and st.session_state.migration_log:
        ...
        if total_new > 0: st.success(f"🎉 成功捕捉并安全合并了 {total_new} 条历史断点记录！")
        else: st.warning("ℹ️ 未发现缺失的历史连续层")
    ...
    if st.button("🚀 启动全员双向穿透吞噬", type="primary"):
        ...
        for idx, code in enumerate(all_codes):
            ...
            live_status.info(f"⏳ 正在核准 [{code}] {fname} 前端同步...")
            local_db, new_front_count, _ = move_ants_front_latest(code)
            
            live_status.info(f"⛏️ 正在穿透 [{code}] 日期大墙向深处挖掘...")
            ...
            log_lines.append(f"**[{code}]** {debug_info}：前台补漏 +{new_front_count}，深度扩充 +{new_deep_count} (总存量: {len(load_local_history(code))}条)")
    ...
    st.markdown(f"**混贴文本清洗流**（不包含6位代码的数据默认流向当前选择：**{current_info['name']}**）")
    raw_text = st.text_area("在此粘贴网页、Excel或终端中复制的数据流水", height=150)
    if st.button("⚡ 自动解析注入", type="primary"):
        if not raw_text.strip(): st.error("⚠️ 未发现可处理输入")
        else:
            ...
            for c, cnt in import_details.items():
                if cnt > 0:
                    ...
                    total += cnt
            if total > 0:
                st.success("🎉 成功分流存盘：" + " ".join(f"[{c}]:+{n}条" for c, n in import_details.items() if n > 0))
                st.rerun()
            else: st.error("❌ 未匹配到可合法的‘日期 净值’结构数据")
    ...
    with c_add:
        ...
        with st.form("add_form"):
            ...
            add_index = st.text_input("标的指数名")
            add_period = st.text_input("定投周期", value="每周二")
            add_amount = st.number_input("定投面额", value=100, step=10)
            if st.form_submit_button("确认创建新档案"):
                if len(add_code) != 6 or not add_name: st.error("⚠️ 输入无效")
                else:
                    ...
                    save_config(st.session_state.fund_config)
                    st.success(f"✅ [{add_code}] 空白资产卡片已创建")
                    st.rerun()

    with c_del:
        st.markdown("##### 🗑️ 物理擦除资产")
        del_target = st.selectbox("选择擦除目标", list(st.session_state.fund_config.keys()), format_func=lambda x: f"[{x}] {st.session_state.fund_config[x]['name']}")
        confirm = st.checkbox("🔥 我知晓该操作会同步删除本地全部JSON历史数据且不可逆")
        if st.button("彻底安全抹除", disabled=not confirm):
...
if current_db:
    ...
    time_frame = st.radio("时间尺度视窗", ["近1个月", "近3个月", "近6个月", "近1年", "全部历史"], horizontal=True, index=4)
    ...
    if not df_f.empty:
        ...
        with c_ck: manual_y = st.checkbox("强制锁定Y轴数值边界", value=False)
        cur_min, cur_max = float(df_f['单位净值'].min()), float(df_f['当月最高价边界'].max())
        pad = (cur_max - cur_min) * 0.08 if cur_max != cur_min else 0.05
        with c_mn: y_min = st.number_input("坐标轴底限", value=round(cur_min - pad, 2), step=0.02, disabled=not manual_y)
        with c_mx: y_max = st.number_input("坐标轴顶限", value=round(cur_max + pad, 2), step=0.02, disabled=not manual_y)
    ...
    tab_y, tab_m, tab_d = st.tabs(["📅 年度历史归档", "🌙 月度价格中枢", "📄 逐日细分明细"])
    with tab_y:
        df_year = df_raw.groupby('年份').agg(流水天数=('日期', 'count'), 累计波动率=('净值增长率', 'sum'), 峰值净值=('单位净值', 'max'), 谷值净值=('单位净值', 'min')).reset_index().sort_values('年份', ascending=False)
        df_year['累计波动率'] = df_year['累计波动率'].map(lambda x: f"{x:+.2f}%")
        st.dataframe(df_year, use_container_width=True, hide_index=True)
    with tab_m:
        df_month = df_raw.groupby('月份').agg(价格中枢=('单位净值', 'mean'), 月度最高价=('单位净值', 'max'), 区间累计波动=('净值增长率', 'sum')).reset_index().sort_values('月份', ascending=False)
        df_month['价格中枢'] = df_month['价格中枢'].map(lambda x: f"{x:.4f}")
        df_month['月度最高价'] = df_month['月度最高价'].map(lambda x: f"{x:.4f}")
        df_month['区间累计波动'] = df_month['区间累计波动'].map(lambda x: f"{x:+.2f}%")
    ...
    💡 当前资产本地数据库为空。<br>请使用上方控制台 <strong>【📋 批量导入】</strong> 直接混贴历史净值，或利用 <strong>【🔄 智能搬家】</strong> 引擎全自动捕获历史层。
    </div>
    """, unsafe_allow_html=True)
